...
public class ChessMatch {
	...
	private List<Piece> capturadePieces = new ArrayList<>();
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		if (testCheck(currentPlayer)) {
			...
			throw new ChessException("You can´t put yourselvf in check ");
		}
	...
	private void validateSourcePosition(Position position) {
		...
		if (!board.piece(position).isThereAnyPossibleMove()) {
			throw new ChessException("Thre is no possible moves for chosen piece");
		}
	...
	private Piece makeMove(Position source, Position target) {
		...
		if (capturedPiece != null) {
			...
			capturadePieces.add(capturedPiece);
		}

		// SpecialMove CastlingKingside rook
		...
		// SpecialMove Castlingquenn rook
		if (p instanceof King && target.getColumn() == source.getColumn() - 2) {
	...
	private void undoMove(Position source, Position target, Piece capturadPiece) {
		...
		if (capturadPiece != null) {
			...
			capturadePieces.remove(capturadPiece);
	...
	private boolean testCheck(Color color) {
		Position kingPosition = king(color).getChessPositione().toPosition();
	...
	private boolean testCheckMate(Color color) {
		...
		for (Piece p : list) {
			...
			for (int i = 0; i < board.getRows(); i++) {
				for (int j = 0; j < board.getColumns(); j++) {
					if (mat[i][j]) {
						Position source = ((ChessPiece) p).getChessPositione().toPosition();
